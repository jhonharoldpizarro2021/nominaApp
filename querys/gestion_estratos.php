      ...
      if( $con ) //Conexion establecida
      {
        switch ( $_POST["opcion"] )
        {
          case 1: //Agregar nuevo usuarios
              ...
              $query = "SELECT COUNT(*) FROM estratos WHERE nombre='". $_POST["estrato"] ."'";
              ...
              if( $row[0] > 0 ) //El correo ya se encuentra registrado
              {
                ...
                $retorno["msg"] = "Estrato ya Existe";
              }else
              {
                $query = "INSERT INTO estratos (nombre)
                          VALUES('". utf8_decode($_POST["estrato"]) ."')";
          ...
          case 3: //Actualizar usuario
              //Verificar que el correo no está registrado
              $query = "SELECT COUNT(*) FROM estratos WHERE nombre='". $_POST["nombre"] ."' AND id_estratos!='". $_POST["id_estratos"] ."'";
              $result = mysqli_query($con,$query);
              $row = mysqli_fetch_array($result);
              ...
              {
                $query = "UPDATE estratos SET nombre='". utf8_decode($_POST["nombre"]) ."' WHERE id_estratos='". $_POST["id_estratos"] ."'";
                if( mysqli_query($con,$query) )
                {
                  $retorno["status"] = "OK";
                }else{
          ...
          case 4://Eliminar Tipo de Documento
              $query = "DELETE FROM estratos WHERE id_estratos='". $_POST["id_estratos"] ."'";
              if( mysqli_query($con,$query) )
              {
                $retorno["status"] = "OK";
              }else{
