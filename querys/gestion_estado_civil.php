      ...
      if( $con ) //Conexion establecida
      {
        switch ( $_POST["opcion"] )
        {
          case 1: //Agregar nuevo usuarios
              ...
              $query = "SELECT COUNT(*) FROM estado_civil WHERE nombre='". $_POST["estado"] ."'";
              ...
              if( $row[0] > 0 ) //El correo ya se encuentra registrado
              {
                ...
                $retorno["msg"] = "Estado Civil ya Existe";
              }else
              {
                $query = "INSERT INTO estado_civil (nombre)
                          VALUES('". utf8_decode($_POST["estado"]) ."')";
          ...
          case 3: //Actualizar usuario
              //Verificar que el correo no está registrado
              $query = "SELECT COUNT(*) FROM estado_civil WHERE nombre='". $_POST["nombre"] ."' AND id_estado_civil!='". $_POST["id_estado_civil"] ."'";
              $result = mysqli_query($con,$query);
              $row = mysqli_fetch_array($result);
              ...
              {
                $retorno["status"] = "EXIST";
                $retorno["msg"] = "Estado Civil registrado";
              }else
              {
                $query = "UPDATE estado_civil SET nombre='". utf8_decode($_POST["nombre"]) ."' WHERE id_estado_civil='". $_POST["id_estado_civil"] ."'";
                if( mysqli_query($con,$query) )
                {
                  $retorno["status"] = "OK";
                }else{
            ...
            break; 
          case 4://Eliminar Estado Civil
              $query = "DELETE FROM estado_civil WHERE id_estado_civil='". $_POST["id_estado_civil"] ."'";
              if( mysqli_query($con,$query) )
              {
                $retorno["status"] = "OK";
              }else{
                $retorno["status"] = "ERROR";
                $retorno["msg"] = "Error al borrar el Estado Civil: ". mysqli_error($con);
              }
            break;
        }
        //Close BD Connection
