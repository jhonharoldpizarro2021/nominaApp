...
function recargarDatos()
{
  $.ajax(
  {
    ...
    success: function(res)
    {
      ...
      {
        var datos = [];
        for( var i=0; i < res.tipo.length; i++)
        {
            var link_editar = '<a href="#" class="fa fa-pencil-square-o" aria-hidden="true" onclick="editar(\''+ res.tipo[i].id +'\',\''+ res.tipo[i].nombre +'\')"></a>';
            var link_borrar = '<a href="#" class="fa fa-trash-o" aria-hidden="true" onclick="eliminar(\''+ res.tipo[i].id +'\',\''+ res.tipo[i].nombre +'\')"></a>';
            datos[i] = [
                            res.tipo[i].nombre,
                            link_editar,
                            link_borrar
    ...
    $.ajax(
    {
        ...
        success: function(res)
        {
            if( res.status === "OK" )
            {
              ...
              jAlert('Tipo de Documento creado con éxito', 'Nuevo Tipo de Documento', function(){
                recargarDatos();
              });
            }else if(res.status === "EXIST" )//Correo existente
            {
              jAlert('El Tipo de Documento '+ tipo +' ya se encuentra registrado', 'Error', function(){
                $("#new_nombre").focus();
              });
            }else if(res.status === "EXPIRED" )//Sesión finalizada
            {
...
/**
* Función encargada de inicializar y abrir PopUp modificar Tipo de Documento
*/
function editar(id,nombre)
{
  //Asignar valores a los campos
  $("#edit_id_tipos").val(id);
    ...
    $.ajax(
    {
        ...
        success: function(res)
        {
            if( res.status === "OK" )
            {
              ...
              jAlert('Tipo de Documento actualizado con éxito', 'Editar Tipo de Documento', function(){
                recargarDatos();
              });
            }else if(res.status === "EXIST" )//Correo existente
            {
              jAlert('El Tipo de Documento '+ nombre +' ya se encuentra registrado', 'Error', function(){
                $("#edit_nombre").focus();
              });
            }else if(res.status === "EXPIRED" )//Sesión finalizada
            {
...
/**
* Función encargada de borrar el Tipo de Documento
*/
function eliminar(id,nombre)
{
  jConfirm('¿Desea borrar el Tipo de Documento:  ' + nombre + ' ?','Eliminar Tipo Documento', function(res){
    if(res)
    {
      $.ajax(
      {
          ...
          success: function(res)
          {
              if( res.status === "OK" )
              {
                jAlert('Tipo de Documento eliminado con éxito', 'Eliminar Tipo de Documento', function(){
                  recargarDatos();
                });
              }else if(res.status === "EXPIRED" )//Sesión finalizada
              {
