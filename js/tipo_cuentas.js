...
function recargarDatos(){
  $.ajax(
  {
    cache: false,
    type: "POST",
    url: "querys/gestion_tipo_cuentas.php",
    data: { 
            opcion : 2 
          },
    dataType: "json",
    success: function(res)
    {
      ...
      {
        var datos = [];
        for( var i=0; i < res.tipo_cuenta.length; i++)
        {
            var link_editar = '<a href="#" class="fa fa-pencil-square-o" aria-hidden="true" onclick="editar(\''+ res.tipo_cuenta[i].id +'\',\''+ res.tipo_cuenta[i].nombre +'\',\''+ res.tipo_cuenta[i].descripcion +'\')"></a>';
            var link_borrar = '<a href="#" class="fa fa-trash-o" aria-hidden="true" onclick="eliminar(\''+ res.tipo_cuenta[i].id +'\',\''+ res.tipo_cuenta[i].nombre +'\')"></a>';
            datos[i] = [
                            res.tipo_cuenta[i].nombre,
                            res.tipo_cuenta[i].descripcion,
                            link_editar,
                            link_borrar
                        ];
        }
    ...
    $.ajax(
    {
        ...
        success: function(res)
        {
            if( res.status === "OK" )
            {
              ...
              jAlert('Tipo de Cuenta actualizada con éxito', 'Editar Parentesco', function(){
                recargarDatos();
              });
            }
            else if(res.status === "EXIST" )//Correo existente
            {
              jAlert('El Tipo de Cuenta '+ nombre +' ya se encuentra registrado', 'Error', function(){
                $("#edit_nombre").focus();
              });
            }
            else if(res.status === "EXPIRED" )//Sesión finalizada
...
function eliminar(id,nombre){
  jConfirm('¿Desea borrar el Tipo de Cuenta:  ' + nombre + ' ?','Eliminar Tipo de Cuenta', function(res){
    if(res)
    {
      $.ajax(
      {
