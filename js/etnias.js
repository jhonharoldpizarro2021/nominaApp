  ...
  $.ajax(
  {
    ...
    success: function(res)
    {
      if( res.status === "OK" )
      {
        ...
        {
            var link_editar = '<a href="#" class="fa fa-pencil-square-o" aria-hidden="true" onclick="editar(\''+ res.etnias[i].id +'\',\''+ res.etnias[i].nombre +'\')"></a>';
            var link_borrar = '<a href="#" class="fa fa-trash-o" aria-hidden="true" onclick="eliminar(\''+ res.etnias[i].id +'\',\''+ res.etnias[i].nombre +'\')"></a>';
            datos[i] = [
                            res.etnias[i].nombre,
                            link_editar,
                            link_borrar
    ...
    $.ajax(
    {
        ...
        success: function(res)
        {
            if( res.status === "OK" )
            {
              ...
              jAlert('Etnia creada con éxito', 'Nueva Etnia', function(){
                recargarDatos();
              });
    ...
    $.ajax(
    {
        ...
        success: function(res)
        {
            if( res.status === "OK" )
            {
              ...
              jAlert('Etnia actualizada con éxito', 'Editar Etnia', function(){
                recargarDatos();
              });
            }else if(res.status === "EXIST" )//Correo existente
            {
              jAlert('La Etnia '+ nombre +' ya se encuentra registrada', 'Error', function(){
                $("#edit_nombre").focus();
              });
            }else if(res.status === "EXPIRED" )//Sesión finalizada
            {
...
{
  jConfirm('¿Desea borrar la Etnia:  ' + nombre + ' ?','Eliminar Etnia', function(res){
    if(res)
    {
      $.ajax(
      {
          ...
          success: function(res)
          {
              if( res.status === "OK" )
              {
                jAlert('Etnia eliminada con éxito', 'Eliminar Etnia', function(){
                  recargarDatos();
                });
              }else if(res.status === "EXPIRED" )//Sesión finalizada
              {
