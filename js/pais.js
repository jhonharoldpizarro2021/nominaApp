...
function recargarDatos()
{
  $.ajax(
  {
    ...
    success: function(res)
    {
      ...
      {
        var datos = [];
        for( var i=0; i < res.pais.length; i++)
        {
            var link_editar = '<a href="#" class="fa fa-pencil-square-o" aria-hidden="true" onclick="editar(\''+ res.pais[i].id +'\',\''+ res.pais[i].nombre +'\')"></a>';
            var link_borrar = '<a href="#" class="fa fa-trash-o" aria-hidden="true" onclick="eliminar(\''+ res.pais[i].id +'\',\''+ res.pais[i].nombre +'\')"></a>';
            datos[i] = [
                            res.pais[i].nombre,
                            link_editar,
                            link_borrar
    ...
    $.ajax(
    {
        ...
        success: function(res)
        {
            if( res.status === "OK" )
            {
              ...
              jAlert('Pais creado con éxito', 'Nuevo Pais', function(){
                recargarDatos();
                $("#new_nombre").val("");
              });
    ...
    $.ajax(
    {
        ...
        success: function(res)
        {
            if( res.status === "OK" )
            {
              ...
              jAlert('Pais actualizado con éxito', 'Editar Pais', function(){
                recargarDatos();
                $("#edit_nombre").val("");
              });
            }else if(res.status === "EXIST" )//Correo existente
            {
              jAlert('El Pais '+ nombre +' ya se encuentra registrado', 'Error', function(){
                $("#edit_nombre").focus();
              });
            }else if(res.status === "EXPIRED" )//Sesión finalizada
            {
...
{
  jConfirm('¿Desea borrar el Pais:  ' + nombre + ' ?','Eliminar Pais', function(res){
    if(res)
    {
      $.ajax(
      {
          ...
          success: function(res)
          {
              if( res.status === "OK" )
              {
                jAlert('Pais eliminado con éxito', 'Eliminar Pais', function(){
                  recargarDatos();
                });
              }else if(res.status === "EXPIRED" )//Sesión finalizada
              {
